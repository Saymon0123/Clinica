 ...
 *
 * Quando o bloqueio é o fim do teste sem documento de quem paga (M2, 0156),
 * mandar o dono "regularizar a assinatura" seria beco: não há cobrança nenhuma
 * para pagar. A tela pede o CPF ou CNPJ ali mesmo, e o banco destrava no
 * instante em que ele é salvo. Só o dono da unidade pode salvar (é a policy da
 * tabela); para o resto da equipe, o recado é chamar o dono.
 */
export function AcessoBloqueado({
  ...
  const { isManager, ehDonoDesta, salonId, salonName } = useSalon()
  const faltaDocumento = assinatura.motivoDoBloqueio === 'sem_documento'
      ...
      <div className="w-full max-w-md space-y-4">
        <div className="bg-surface border border-border rounded-2xl shadow-sm p-5">
          ...
          <h1 className="text-xl font-bold tracking-tight text-foreground">
            {faltaDocumento ? 'O teste grátis acabou' : 'Acesso ao CRM suspenso'}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {salonName ? `${salonName} — ` : ''}
            {faltaDocumento
              ? 'para continuar, falta o CPF ou CNPJ de quem vai pagar.'
          ...
          {faltaDocumento ? (
            ehDonoDesta ? (
              <p className="text-sm text-muted-foreground mt-4">
                É com ele que a cobrança do mês é emitida. Cadastre logo abaixo: o acesso volta na hora.
              </p>
            ) : (
              /* A policy só deixa o dono gravar o documento: mostrar o campo a
                 quem não pode salvar seria um erro garantido. */
              <p className="text-sm text-muted-foreground mt-4">
                Só o dono da barbearia pode cadastrar o documento. Avise o dono para liberar o acesso de
                novo.
              </p>
            )
          ) : isManager ? (
        ...
        {faltaDocumento && ehDonoDesta && salonId && (
          <DadosDeCobranca salonId={salonId} documentoAtual={null} onSalvo={onRegularizou} />
        )}
      </div>
    </div>
  )
