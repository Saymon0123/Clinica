 ...
 *
 * Sem o documento de quem paga, o teste acaba e o acesso não renova (M2, 0156).
 * O pedido vem junto com o aviso, porque é aqui que ele importa — e é o único
 * aviso que chega de fato: o de WhatsApp depende de um modelo que a Meta ainda
 * não aprovou.
 */
export function AvisoAssinatura({ assinatura }: { assinatura: Assinatura | null }) {
  ...
  const texto = expirada
    ? assinatura.motivoDoBloqueio === 'sem_documento'
      ? 'Seu período de teste terminou. O atendimento pelo WhatsApp continua funcionando.'
